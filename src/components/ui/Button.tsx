...
export function Button({ title, loading = false, variant = 'primary', disabled, style, ...props }: ButtonProps) {
  ...
  return (
    <Pressable {...props} disabled={isDisabled} style={({ pressed }) => [containerStyles, pressed && styles.pressed]}>
      {loading ? (
